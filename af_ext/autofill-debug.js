...
const DEBUG_STORAGE_KEY = "autofillDebugEnabled";
const TOGGLE_SHORTCUT = "Ctrl+Alt+Shift+D";
...
function applyDebugState(nextState, source) {
	...
	if (debugEnabled) {
		...
		console.info(
			`[AutoFill Debug] ativo (${source}). Atalho: ${TOGGLE_SHORTCUT}. Desative repetindo o atalho.`
		);
		return;
	}
...
function onScoreEvent(event) {
	...
	console.groupCollapsed(
		`[AutoFill Score] ${identity} -> ${key} | score ${score}/${threshold} | fill ${detail.filled ? "sim" : "nao"}`
	);
